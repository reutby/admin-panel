<template>
	<el-form class="storage-form" @submit.native.prevent="submit">
		<FormInput title="Name" :value="editedStorage.name" @input="editedStorage.name = $event"/>
		<FormInput title="Public URL" :value="editedStorage.metadata.publicUrl"
		           @input="editedStorage.metadata.publicUrl = $event"/>
		<FormInput title="Base Storage Path" :value="editedStorage.metadata.basePath"
		           @input="editedStorage.metadata.basePath = $event"/>
		<p>
			<label>
				Kind:
				<el-select :value="editedStorage.kind" @change="editedStorage.kind = $event">
		...
		<div class="authentication">
			...
			<template v-if="showAuth">
				<StorageFtpAuth v-if="editedStorage.kind === 'ftp'"
